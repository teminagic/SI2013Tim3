...
public class BossController {
	...
	public void control() {
		...
		odjava.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					if(JOptionPane.showOptionDialog(view, "Eventualne promjene nece biti spa≈°ene. "
...
=======
		...
		izvjestajIstekleRegistracije.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					ShowDialogInput dialog = new ShowDialogInput("Unesite lokaciju za spremanje izvjeötaja:", 1);
		...
		izvjestajDozvole.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					ShowDialogInput dialog = new ShowDialogInput("Unesite lokaciju za spremanje izvjeötaja:", 2);
				} catch (ParseException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
	...
	void pretraziVozila() {
		...
		if(parametar.equals("")) {
			...
			if(model.pretragaVozilo(parametar,kriterij.toString()) == null) {
				view.getStrana2().getPanel_vozilo().getModel().clearAll();
				JOptionPane.showOptionDialog(view, "Nije pronaÔøΩeno nijedno vozilo s unesenim parametrima.",
						"Upozorenje.", JOptionPane.OK_OPTION,
						JOptionPane.INFORMATION_MESSAGE, null,
						new String[] { "Uredu" }, "default");
			}
